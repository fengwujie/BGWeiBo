...
- (void)viewDidLoad {
    ...
    for (int i=0; i<BGNewfeatureCount; i++) {
        ...
        // 如果是最后一个imageView，就往里面添加其他内容（分享微博和开始微博）
        if (i == BGNewfeatureCount-1 ) {
...
#warning 就算不主动添加子控件到scrollView中，scrollView内部还是可能会有一些子控件
    ...
    scrollView.contentSize = CGSizeMake(BGNewfeatureCount * scrollW, 0);
    scrollView.bounces = NO; // 去除弹簧效果
    scrollView.pagingEnabled = YES;  //分页效果
    scrollView.showsHorizontalScrollIndicator = NO;  //垂直方向滚动
    scrollView.delegate = self;
    
    // 4.添加pageControl：分页，展示目前看的是第几页
    UIPageControl *pageControl = [[UIPageControl alloc] init];
    pageControl.numberOfPages = BGNewfeatureCount;
    ...
    //pageControl.userInteractionEnabled=NO;   //设置页数的圆圈不能触摸事件，如果没有设置高度，就可以不用设置这个属性
...
{
    // 开启交互功能，UIImageView默认为NO，其上面的控件无法触发事件
