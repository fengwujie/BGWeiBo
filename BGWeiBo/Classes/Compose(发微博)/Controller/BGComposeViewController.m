...
/** 输入控件 */
@property (nonatomic, weak) BGTextView *textView;
/** 键盘顶部的工具条 */
...
- (void)dealloc
{
    [BGNotificationCenter removeObserver:self];
}
...
- (void)setupTextView
{
    ...
    BGTextView *textView = [[BGTextView alloc] init];
    // 垂直方向上永远可以拖拽（有弹簧效果）
    ...
    // 文字改变的通知
    [BGNotificationCenter addObserver:self selector:@selector(textDidChange) name:UITextViewTextDidChangeNotification object:textView];
    
    // 键盘通知
    // 键盘的frame发生改变时发出的通知（位置和尺寸）
    [BGNotificationCenter addObserver:self selector:@selector(keyboardWillChangeFrame:) name:UIKeyboardWillChangeFrameNotification object:nil];
}
...
- (void)sendWithImage
{
    ...
    params[@"access_token"] = [BGAccountTool account].access_token;
...
#pragma mark - BGComposeToolbarDelegate
